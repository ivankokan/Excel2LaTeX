...
Dim name As String, caption As String, i As Long
Dim ControlCollection As New Collection
Dim myMenubar, toolsMenu, newMenuItem, newButton

name = "Excel2LaTeX"
caption = "Convert Table to Latex"
'First Create Menu Item
Set myMenubar = Application.CommandBars.ActiveMenuBar
Set toolsMenu = myMenubar.Controls(6)
Set newMenuItem = myMenubar.FindControl(Tag:=name, recursive:=True)
...
newMenuItem.Tag = name

'Now create tool bar
Dim myToolBar
On Error GoTo barDidNotexist
  Set myToolBar = Application.CommandBars(name)
  If myToolBar.Controls.Count > 0 Then myToolBar.Controls(1).Delete
  GoTo continue
barDidNotexist:
  Set myToolBar = Application.CommandBars.Add(name:=name)
continue:
On Error GoTo 0
...
Set newButton = myToolBar.Controls.Add(msoControlButton)
  ...
  For Each ctl In ControlCollection
    ctl.OnAction = "Latex"
    ctl.FaceId = 107
    ctl.TooltipText = "Convert Table to Latex"
    ctl.caption = caption
  Next ctl
done:
End Sub
